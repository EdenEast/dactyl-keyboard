...
def minidox_thumb_connectors():
    ...
    # For some reason there is an issue with cadquery unioning this geometry. The result is an empty list.
    # The only reason that cadquery is required is for generating the plate. The plate can be generated by
    # adding the hulls instead of unioning them. To generate the actual model use solid

    return add(hulls) if ENGINE == 'cadquery' else union(hulls)
